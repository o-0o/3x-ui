...
echo "Arch: $(arch)"
if [[ "$(arch)" != "amd64" ]]; then
    _fail "ERROR: This forked tokenbucket build currently publishes linux amd64 only."
fi
...
config_after_update() {
    ...
    # Updates preserve the existing panel configuration. SSL, webBasePath and
    # listenIP remain explicit install/menu choices.
    ...
    if [[ $release == "alpine" ]]; then
        ...
        if [ -f "x-ui.service" ]; then
            ...
            if [ "$service_installed" = false ]; then
                ...
                case "${release}" in
                    ubuntu | debian | armbian)
                        ${curl_bin} -fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/${repo_owner}/${repo_name}/${repo_branch}/x-ui.service.debian > /dev/null 2>&1
                        ;;
                    arch | manjaro | parch)
                        ${curl_bin} -fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/${repo_owner}/${repo_name}/${repo_branch}/x-ui.service.arch > /dev/null 2>&1
                        ;;
                    *)
                        ${curl_bin} -fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/${repo_owner}/${repo_name}/${repo_branch}/x-ui.service.rhel > /dev/null 2>&1
                        ;;
                esac
