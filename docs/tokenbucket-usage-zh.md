...
```bash
v3.4.1-tokenbucket.4
```

Release 地址：

```text
https://github.com/o-0o/3x-ui/releases/tag/v3.4.1-tokenbucket.4
```
...
```bash
bash <(curl -Ls https://raw.githubusercontent.com/o-0o/3x-ui/feature/xray-core-tokenbucket-speed-limit/update.sh) v3.4.1-tokenbucket.4
```
...
```bash
bash <(curl -Ls https://raw.githubusercontent.com/o-0o/3x-ui/v3.4.1-tokenbucket.4/install.sh) v3.4.1-tokenbucket.4
```
...
例如当前版本是 `v3.4.1-tokenbucket.4`，下一版可以使用 `.5`：

```bash
TAG=v3.4.1-tokenbucket.5
git tag -a "$TAG" -m "$TAG"
git push origin "$TAG"
```
