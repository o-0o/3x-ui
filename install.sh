...
echo "Arch: $(arch)"
if [[ "$(arch)" != "amd64" ]]; then
    echo -e "${red}This forked tokenbucket build currently publishes linux amd64 only.${plain}"
    exit 1
fi
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        curl -fLR --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 300 -o ${xui_folder}-linux-$(arch).tar.gz https://github.com/${repo_owner}/${repo_name}/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
    ...
    curl -fLRo /usr/bin/x-ui-temp https://raw.githubusercontent.com/${repo_owner}/${repo_name}/${repo_branch}/x-ui.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        curl -fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/${repo_owner}/${repo_name}/${repo_branch}/x-ui.rc
        if [[ $? -ne 0 ]]; then
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl -fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/${repo_owner}/${repo_name}/${repo_branch}/x-ui.service.debian > /dev/null 2>&1
                    ;;
                arch | manjaro | parch)
                    curl -fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/${repo_owner}/${repo_name}/${repo_branch}/x-ui.service.arch > /dev/null 2>&1
                    ;;
                *)
                    curl -fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/${repo_owner}/${repo_name}/${repo_branch}/x-ui.service.rhel > /dev/null 2>&1
                    ;;
            esac
