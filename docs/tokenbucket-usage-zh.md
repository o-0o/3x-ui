...
```bash
v3.4.0-tokenbucket.3
```

Release 地址：

```text
https://github.com/o-0o/3x-ui/releases/tag/v3.4.0-tokenbucket.3
```
...
```bash
bash <(curl -Ls https://raw.githubusercontent.com/o-0o/3x-ui/feature/xray-core-tokenbucket-speed-limit/update.sh) v3.4.0-tokenbucket.3
```
...
```bash
bash <(curl -Ls https://raw.githubusercontent.com/o-0o/3x-ui/v3.4.0-tokenbucket.3/install.sh) v3.4.0-tokenbucket.3
```
...
例如当前版本是 `v3.4.0-tokenbucket.3`，下一版可以使用 `.4`：

```bash
TAG=v3.4.0-tokenbucket.4
git tag -a "$TAG" -m "$TAG"
git push origin "$TAG"
```
