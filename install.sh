...
install_x-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls "https://api.github.com/repos/o-0o/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        url="https://github.com/o-0o/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
    ...
    curl -4fLRo /usr/bin/x-ui-temp https://raw.githubusercontent.com/o-0o/3x-ui/feature/dynamic-bandwidth-balancer/x-ui.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        curl -4fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/o-0o/3x-ui/feature/dynamic-bandwidth-balancer/x-ui.rc
        if [[ $? -ne 0 ]]; then
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/o-0o/3x-ui/feature/dynamic-bandwidth-balancer/x-ui.service.debian > /dev/null 2>&1
                    ;;
                arch | manjaro | parch)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/o-0o/3x-ui/feature/dynamic-bandwidth-balancer/x-ui.service.arch > /dev/null 2>&1
                    ;;
                *)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/o-0o/3x-ui/feature/dynamic-bandwidth-balancer/x-ui.service.rhel > /dev/null 2>&1
                    ;;
            esac
