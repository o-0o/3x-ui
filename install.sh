...
install_x-ui() {
    cd ${xui_folder%/x-ui}/
    if [[ "$(arch)" != "amd64" ]]; then
        echo -e "${red}This forked speed-limit build currently publishes linux amd64 only.${plain}"
        exit 1
    fi
    ...
    curl -4fLRo /usr/bin/x-ui-temp https://raw.githubusercontent.com/${repo_owner}/${repo_name}/${repo_branch}/x-ui.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        curl -4fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/${repo_owner}/${repo_name}/${repo_branch}/x-ui.rc
        if [[ $? -ne 0 ]]; then
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/${repo_owner}/${repo_name}/${repo_branch}/x-ui.service.debian > /dev/null 2>&1
                    ;;
                arch | manjaro | parch)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/${repo_owner}/${repo_name}/${repo_branch}/x-ui.service.arch > /dev/null 2>&1
                    ;;
                *)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/${repo_owner}/${repo_name}/${repo_branch}/x-ui.service.rhel > /dev/null 2>&1
                    ;;
            esac
